...
export default class BoundarySelector {
     ...
     * @param {number} i - 索引数组的位置下标（0 ≤ i < n）
     * @returns {number} 对应位置存储的边界索引值
     */
     ...
     * @param {number} index - 需要选中的边界索引值（需存在于indices数组中）
     */
     ...
     * @param {number} size - 分割阈值（有效索引的上限）
     */
